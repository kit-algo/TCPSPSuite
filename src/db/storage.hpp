...
class Storage {
public:
  explicit Storage(std::string filename, unsigned int retry_count = 10);
  ...
  bool check_result(const std::string & instance_id, const std::string & run_id,
	...
	std::vector<std::shared_ptr<DBResult>> get_results_for_config(const SolverConfig & sc);
...
private:
  ...
  std::vector<unsigned long> find_db_configs(const SolverConfig & sc, bool ignore_name = false);
  /*
