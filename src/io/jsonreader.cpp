...
JsonReader::parse()
{
  ...
  Instance * instance =
      new Instance(this->get_json<std::string>("id"), Traits());
  ...
  for (unsigned int i = 0; i < js["jobs"].size(); i++) {
    ...
    Job job(job_data[i]["release"], job_data[i]["deadline"],
            job_data[i]["duration"], usages, 0);
  ...
  for (unsigned int i = 0; i < js["jobs"].size(); i++) {
    ...
    for (json::iterator it = job_data[i]["successors"].begin();
