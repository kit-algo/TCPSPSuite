...
SolverConfig::SolverConfig() : l("SCFG") {}

SolverConfig::SolverConfig(
    std::string name_in, std::string id_str_in,
    std::map<std::string, json::basic_json::value_type> values_in,
    Maybe<unsigned int> time_limit_in, bool enable_memory_metrics_in,
    unsigned int meminfo_sampling_time_in,
    std::vector<std::string> papi_metrics_in, Maybe<int> seed_in)
    : values(values_in), name(name_in), id_matcher(id_str_in),
      id_str(id_str_in), time_limit(time_limit_in),
      enable_memory_metrics(enable_memory_metrics_in),
      meminfo_sampling_time(meminfo_sampling_time_in),
      papi_metrics(papi_metrics_in), seed(seed_in), l("SCFG")
{
...
bool
SolverConfig::as_bool(const std::string & key) const
{
	std::string val = this->values.at(key);
	return val.compare("yes") == 0;
}
...
SolverConfig::check_all_keys_queried() const
{
	for (const auto & kv : this->values) {
		if (this->requested_keys.find(kv.first) == this->requested_keys.end()) {
			BOOST_LOG(l.w()) << "SolverConfig has key " << kv.first
			                 << ", which was not queried.";
		}
	}
}

void
SolverConfig::override_config(const std::string & key,
...
SolverConfig::read_configs(const std::string & filename)
{
...
SolverConfig::read_configs(json js)
{
	...
	for (const auto & entry : js["solvers"]) {
		try {
			id_str = entry.at("regex");
			name_str = entry.at("name");
		} catch (json::out_of_range & e) {
			Log l("SCONFIG");
			BOOST_LOG(l.e()) << "Missing key in solver config: " << e.what();
			throw e;
		}
		...
		if (entry.find("meminfo_sampling_time") != entry.end()) {
			...
			meminfo_sampling_time =
			    (unsigned int)std::stoul(meminfo_sampling_time_str);
		} else {
		...
		if ((entry.find("config") != entry.end()) && (entry["config"].size() > 0)) {
			for (auto element_it = entry["config"].begin();
			     element_it != entry["config"].end(); ++element_it) {
				values[element_it.key()] = element_it.value();
			}
		}

		SolverConfig sc{name_str,
		                id_str,
		                values,
		                time_limit,
