...
EBILP<SolverT>::compute_skip_numbers() noexcept
{
	...
	for (unsigned int jid = 0; jid < this->instance.job_count(); ++jid) {
		this->skip_numbers[jid] = {predecessors[jid], successors[jid]};
		this->event_counts[jid] =
		    this->instance.job_count() + 1 - predecessors[jid] - successors[jid];
...
EBILP<SolverT>::prepare_variables()
{
	...
	for (unsigned int jid = 0; jid < this->instance.job_count(); ++jid) {
		size_t necessary_events = event_count - this->skip_numbers[jid].first -
		                          this->skip_numbers[jid].second;
