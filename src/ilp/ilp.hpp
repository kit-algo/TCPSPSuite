 ...
 * start-indicator variable is only created up to the point where the last job
 * must start the latest.
 */
template <class MIPSolverT>
class ILPBase {
public:
	ILPBase(const Instance & instance, AdditionalResultStorage & additional,
...
protected:
	...
	public:
		Callback(const Timer & timer, AdditionalResultStorage & additional_storage,
		         Log & l);
		virtual void on_poll(typename MIPSolver::Callback::Context & ctx) override;
		virtual void on_message(typename MIPSolver::Callback::Context & ctx,
	 ...
	 * If they are generated, implementation must make sure to introduce
	 * constraints on them! */
