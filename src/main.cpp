...
bool has_cmd_option(char ** begin, char ** end, const std::string & option)
{
  char ** itr = std::find(begin, end, option);
  if (itr != end)
  {
...
void print_help() {
  std::cout << "Usage: tcpsp -s <storage> [ -f <instance file> | -d <instance directory> ] OPTIONS \n";
  std::cout << "\n";
  ...
  std::cout << "              : Specifies the path to a single JSON algorithm configuration\n";
  std::cout << "                file. This may not be used in conjunction with -a. All algorithms\n";
  std::cout << "                specified in the config file will be executed on all instances with the\n";
  std::cout << "                specified configurations.\n";
  std::cout << "\n";
  std::cout << "  -a <algorithm regexp>\n";
  std::cout << "              : Specifies a regular expression to match against the algorithm IDs. All\n";
  std::cout << "                matching algorithms will be executed on all instances. The algorithms\n";
  std::cout << "                must be configured with the optional parameters below. This may not be used\n";
  ...
  std::cout << "   > tcpspsuite -f /tmp/instance.json -s /tmp/db.sdlite -r testrun\n";
  std::cout << "\n";
  ...
  std::cout << "   > tcpspsuite -d /tmp/instances -s /tmp/db.sdlite -l 900 -u\n";
  std::cout << "\n";
}
...
int main(int argc, const char **argv) {
  ...
  if (cfg.get_algo_regexp().valid()) {
      BOOST_LOG(l.d(3)) << "Adding Ad-Hoc SC with regexp " << std::string(cfg.get_algo_regexp());
    SolverConfig sc("ADHOC",
                    std::string(cfg.get_algo_regexp()),
                    {}, cfg.get_time_limit(), cfg.get_instance_seed());
    cfg.set_solver_config(sc);
      BOOST_LOG(l.d(3)) << "We have " << cfg.solver_configs().size() << " solver configs.";
  } else {
