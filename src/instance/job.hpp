 ...
 *   - A duration: Specifies for how long the job must be executed (without
 * being interrupted).
 *   - A release: Specifies the first time step in which the job may be
 * executed.
 *   - A deadline: Specifies the first time step in which the job must be
 * finished.
 *   - One usage specification per resource: Specifies how much this job uses of
 * a given resource during its execution.
 */
class Job {
public:
	using JobId = unsigned int;

	// TODO actually, this should not be default-constructible. We should adapt
	// the way substitutions work
	Job();

	/**
	 * Constructs a job from its release time, deadline, duration, usages and its
	 * ID.
	 *
	 ...
	 * @param usages    A vector of usage specifications. usages[i] specifies how
	 * much of resource with ID i this job requires.
	 * @param id        The ID of the job to be created.
	 */
	Job(unsigned int release, unsigned int deadline, unsigned int duration,
	    ResVec usages, unsigned int id);

	// Modified copy constructor that allows to change release and deadline during
	// copying
	/**
	 * Creates a job as a copy of another job (with the same ID!), allowing to
	 * modify release and deadline.
	 *
	 * **Warning**: Never add this job to the same instance as the <other> job.
	 * Both jobs have the same ID, and two jobs with the same ID within the same
	 * instance produces undefined behavior.
	 *
	 ...
	 *
	 * **Warning**: Changing the ID of the job after the lag graph has been
	 * initialized breaks the instance. Only use this if you know what you are
	 * doing.
	 *
	...
	/**
	 * Sets the start-time hint of the job. This hint may be used by solvers
	 * to speed up solving.
	 *
	 * @param hint The hint to be used, or Maybe<unsigned int>{} for no hint.
	 */
