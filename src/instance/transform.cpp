...
#include <algorithm>                            // for move, max, min
#include <limits>                               // for numeric_limits
#include <memory>                               // for allocator
#include <set>                                  // for set, _Rb_tree_const_i...
#include <string>                               // for operator+, basic_string
#include <vector>                               // for vector
#include "laggraph.hpp"  // for LagGraph::(anonymous)
...
#include "../instance/traits.hpp"                  // for TraitsRouter, TraitsR...
...
DummyStartEndTransformer::run(const Instance & input)
{
  ...
  for (auto edge : input.get_laggraph().edges()) {
    ...
    this->transformed.get_laggraph().add_edge(s_job, t_job, {edge.lag, edge.drain_factor, edge.max_recharge});
  }
...
/*
  ...
  */
  ...
  std::vector<double> zero_usages(input.resource_count(), 0);
  Job dummy_start_job = Job(min_release, max_deadline + 1, 0, zero_usages, 0);
  unsigned int dummy_start = this->transformed.add_job(std::move(dummy_start_job));
  ...
  for (unsigned int j = 0 ; j < input.job_count() ; ++j) {
    ...
    if (has_incoming_edges.find(j) == has_incoming_edges.end()) {
      this->transformed.get_laggraph().add_edge(dummy_start_job, this->transformed.get_job(j), {0, 0.0, 0});
    }

    if (has_outgoing_edges.find(j) == has_outgoing_edges.end()) {
      this->transformed.get_laggraph().add_edge(this->transformed.get_job(j), dummy_end_job, {0, 0.0, 0});
    }
  }
}
...
SameStartEndTransformer::run(const Instance & input)
{
  ...
  for (auto edge : input.get_laggraph().edges()) {
    ...
    this->transformed.get_laggraph().add_edge(s, t, {edge.lag, edge.drain_factor, edge.max_recharge});
  }
...
*/
  ...
  for (unsigned int i = 0; i < input.job_count(); ++i) {
    ...
    Job preprend_dummy(min_release, max_deadline, (orig_release - min_release), std::vector<double>(this->transformed.resource_count(), 0.0), 0);
    Job append_dummy(min_release, max_deadline, (max_deadline - orig_deadline), std::vector<double>(this->transformed.resource_count(), 0.0), 0);

    unsigned int prepend_index = this->transformed.add_job(std::move(preprend_dummy));
    unsigned int append_index = this->transformed.add_job(std::move(append_dummy));

    const Job &out_job = this->transformed.get_job(i);

    int forward_lag = 0;
    int reverse_lag = (int)in_job.get_duration();
    this->transformed.get_laggraph().add_edge(this->transformed.get_job(prepend_index), out_job, {forward_lag, 0.0, 0});
    this->transformed.get_laggraph().add_edge(out_job, this->transformed.get_job(append_index), {reverse_lag, 0.0, 0});
  }
}
...
std::set<Transformer *> TransformerManager::all_transformers = {
  new SameStartEndTransformer(),
  new DummyStartEndTransformer(),
...
ConsistentWindowTransformer::run(const Instance & input)
{
  ...
  for (unsigned int i = 0; i < input.job_count(); ++i) {
    ...
    unsigned int new_release = std::max(in_job.get_release(), earliest_starts[in_job.get_jid()]);
    unsigned int new_deadline = std::min(in_job.get_deadline(), latest_finishs[in_job.get_jid()]);

    if (new_release + in_job.get_duration() > new_deadline) {
      throw InconsistentDataError(input, -1, FAULT_CRITICAL_PATH_INFEASIBLE, "The critical paths for job " + std::to_string(in_job.get_jid()) + " are infeasible");
    }
