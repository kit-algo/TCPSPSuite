...
const unsigned long Traits::FLAT_OVERSHOOT = 1 << 15;

const char * const Traits::FLAG_NAMES[] = {"NO_LAGS",
...
const std::vector<std::pair<unsigned long, unsigned long>>
    Traits::implications = std::vector<std::pair<unsigned long, unsigned long>>(
...
Traits::Traits(unsigned long flags_in, unsigned int max_resources_in,
               std::set<double> allowed_overshoot_exponents_in,
               std::set<double> allowed_investment_exponents_in)
    : flags(flags_in), max_resources(max_resources_in),
      allowed_overshoot_exponents(allowed_overshoot_exponents_in),
...
Traits::clone() const
{
	Traits cloned(this->flags, this->max_resources,
	              this->allowed_overshoot_exponents,
	              this->allowed_investment_exponents);
...
Traits::fulfills(const Traits & requirements) const
{
	...
	if (requirements.allowed_investment_exponents.size() > 0) {
		...
		for (auto exponent : this->allowed_investment_exponents) {
			if (requirements.allowed_investment_exponents.find(exponent) ==
			    requirements.allowed_investment_exponents.end()) {
				return false;
			}
		}
	}
	...
	if (requirements.allowed_overshoot_exponents.size() > 0) {
		...
		for (auto exponent : this->allowed_overshoot_exponents) {
			if (requirements.allowed_overshoot_exponents.find(exponent) ==
			    requirements.allowed_overshoot_exponents.end()) {
				return false;
			}
		}
	}
...
TraitsBuilder::check_window_extension()
{
	...
	if (this->instance.get_window_extension_job_limit() >=
...
TraitsBuilder::check_availabilities()
{
	...
	for (unsigned int rid = 0; rid < this->instance.resource_count(); ++rid) {
		...
		for (const auto & point : res.get_availability()) {
			if ((point.second > amount + DOUBLE_DELTA) ||
			    (point.second < amount - DOUBLE_DELTA)) {
				all_flat = false;
			}
		}
	}
...
void
TraitsBuilder::check_overshoot()
{
	bool all_flat = true;
	for (unsigned int rid = 0; rid < this->instance.resource_count(); ++rid) {
		const Resource & res = this->instance.get_resource(rid);

		all_flat &= res.is_overshoot_flat();
	}

	if (all_flat) {
		this->flags |= Traits::FLAT_OVERSHOOT;
	}
}
...
{
	// There are three conditions: The graph is connected, has one source, and one
	// sink!
	...
	for (const auto & job : this->instance.get_jobs()) {
		if (this->instance.get_laggraph().reverse_neighbor_count(job.get_jid()) ==
		    0) {
	...
	std::function<bool(LagGraph::vertex, LagGraph::vertex)> visit =
	    [&](LagGraph::vertex v, LagGraph::vertex from) {
		    (void)from;
	...
	std::function<void(LagGraph::vertex, LagGraph::vertex, int)> traverse =
	    [&](LagGraph::vertex from, LagGraph::vertex to, int lag) {
		    (void)from;
		    (void)to;
		    (void)lag;
	    };
	...
	DFS<decltype(visit), decltype(backtrack), decltype(traverse)>(
	    this->instance.get_laggraph(), source, visit, backtrack, traverse, false);
...
TraitsBuilder::check_deadline_release()
{
	...
	if (this->instance.job_count() > 0) {
		...
		for (unsigned int job_id = 1; job_id < this->instance.job_count();
		     ++job_id) {
...
TraitsBuilder::get_traits()
{
	...
	for (unsigned int r_id = 0; r_id < this->instance.resource_count(); ++r_id) {
		const polynomial & overshoot_costs =
		    this->instance.get_resource(r_id).get_overshoot_costs();
		for (const std::pair<double, double> & term : overshoot_costs) {
			overshoot_exponents.insert(std::get<1>(term));
		}

		for (const auto & [point, poly] :
		     this->instance.get_resource(r_id).get_flex_overshoot()) {
			for (const std::pair<double, double> & term : poly) {
				overshoot_exponents.insert(std::get<1>(term));
			}
		}

		polynomial investment_costs =
		    this->instance.get_resource(r_id).get_investment_costs();
		for (std::pair<double, double> & term : investment_costs) {
			investment_exponents.insert(std::get<1>(term));
		}
	}

	return Traits(this->flags, this->instance.resource_count(),
	              overshoot_exponents, investment_exponents);
}
...
bool
TraitsRouter::match(const trait_profile & traits_p,
                    const transform_profile & trans_p) const
{
...
bool
TraitsRouter::fulfills(const trait_profile & profile,
                       const trait_profile & pattern) const
{
...
TraitsRouter::transform_flags(const trait_profile & in_profile,
                              const transform_profile & transform) const
{
...
TraitsRouter::do_bfs(const Traits & from, const Traits & to)
{
	...
	this->tree.insert(
	    std::make_pair(from_profile, std::make_pair(nullptr, from_profile)));
	...
	while ((!this->found) && (this->queue.size() > 0)) {
		...
		for (auto transformer : this->find_matching(profile)) {
			trait_profile out =
			    this->transform_flags(profile, transformer->get_transformation());
			...
			if (this->tree.find(out) == this->tree.end()) {
				...
				this->tree.insert(
				    std::make_pair(out, std::make_pair(transformer, profile)));
				queue.push_back(out);
