...
Solution::print_profile() const
{
	...
	std::vector<decltype(
	    this->instance->get_resource(0).get_availability().begin())>
	    availability_its;
			...
			for (unsigned int r = 0; r < this->instance->resource_count(); ++r) {
				...
				while ((availability_its[r] + 1 !=
					...
					if (subseg_end > last_t) {
						// std::cout << "~~~~~> Sub-Segment in Resource " << r << " up to "
						// << subseg_end << "\n"; std::cout << "Usage: " << current[r] <<
						// "\n";
						double overshoot_here =
						    std::max(0.0, (current[r] - availability_its[r]->second));
						double overshoot_costs_per_timestep = apply_polynomial(
						    this->instance->get_resource(r).get_overshoot_costs(),
						    overshoot_here);
						// std::cout << "Costs per time step: " <<
						// overshoot_costs_per_timestep << "\n";
						...
						// Update maximum
						this->max_usage[r] = std::max(
						    this->max_usage[r], (current[r] - availability_its[r]->second));
				...
				// << "\n";
				double segment_overshoot_costs =
				    (t - subseg_start) * overshoot_costs_per_timestep;
				// std::cout << "Segment start: " << subseg_start << " / Segment End: "
				// << t << "\n"; std::cout << "Segment costs: " <<
				// segment_overshoot_costs << "\n";
				acc_overshoot_costs += std::max(segment_overshoot_costs, 0.0);
