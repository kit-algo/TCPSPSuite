...
public:
	...
	static const unsigned long FLAT_OVERSHOOT;

	// End of flags

	static const char * const FLAG_NAMES[];

	Traits(unsigned long flags, unsigned int max_resources,
...
private:
	...
	static const std::vector<std::pair<unsigned long, unsigned long>>
	    implications;
};
...
const unsigned long ALL_TRAIT_FLAGS[] = {
    Traits::NO_LAGS,
    Traits::LAGS_ONLY_SUCCESSORS,
    Traits::LAGS_ONLY_GREATER_DURATION,
    Traits::LAGS_ONLY_POSITIVE,
    Traits::LAGS_DAG,
    Traits::COMMON_RELEASE,
    Traits::COMMON_DEADLINE,
    Traits::COMMON_DURATION,
    Traits::DUMMY_START_END,
    Traits::NO_DRAIN,
    Traits::CONSISTENT_WINDOWS,
    Traits::NO_WINDOW_EXTENSION,
    Traits::WINDOW_EXTENSION_JOBS_UNLIMITED,
    Traits::FLAT_AVAILABILITY,
    Traits::ZERO_AVAILABILITY,
    Traits::FLAT_OVERSHOOT};
...
public:
	...
	using transform_profile =
	    std::array<unsigned short, get_array_size(ALL_TRAIT_FLAGS)>;

	explicit TraitsRouter(const std::set<Transformer *> & transformers);
	Maybe<std::vector<Transformer *>> get_path(const Traits & from,
	                                           const Traits & to);
...
private:
	...
	std::unordered_map<trait_profile, std::pair<Transformer *, trait_profile>>
	    tree;
	...
	bool match(const trait_profile & traits_p,
	           const transform_profile & trans_p) const;
	bool fulfills(const trait_profile & profile,
	              const trait_profile & pattern) const;
	std::vector<Transformer *> find_matching(const trait_profile & profile) const;
	trait_profile transform_flags(const trait_profile & in_profile,
...
flag_to_index(unsigned long flag)
{
